...
public class Exercises {
    ...
    private static void exercise2() {
        ...
        List<Double>mapCosts=new ArrayList<>();
        //Forse esisteva un modo più compatto di farlo, alle 2 di notte non mi è venuto in mente

        for (Map.Entry<String,Double>costs :menuCostsInEuro.entrySet()){

            mapCosts.add(costs.getValue());

        }

        for (Map.Entry<String,Double> mapElement :menuCostsInEuro.entrySet()){


            if(mapElement.getValue().equals(Collections.max(mapCosts))){

                mostExpensiveMenuItemName=mapElement.getKey();
    ...
    private static void exercise3() {
        ...
        for (Map.Entry<String,Integer> weights : fruitWeights.entrySet()){

            totalWeight+=weights.getValue();

        }


        if (totalWeight == fruitWeights.values().stream().mapToInt(Integer::intValue).sum()) {//Immagino questa sia la maniera furba di farlo
